...
6. **技能区必须认真处理**：从全文（含项目技术栈）回填已有依据的关键词；同技术不同写法由模型语义判断；无故整段原样视为不合格；全文没有的词禁止写入。
...
- 若关键词在项目技术栈有、专业技能区没有：记为 **matched**，并可在 `weak_points` / `next_steps` 建议「回填专业技能区便于 ATS 检索」；**禁止**因此标成缺失
